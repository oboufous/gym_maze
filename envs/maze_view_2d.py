...
class MazeView2D:
    ...
    def __init__(self, maze_name="Maze2D", maze_file_path=None,
                 maze_size=(30, 30), screen_size=(200, 200),
                 has_loops=True, num_portals=0, enable_render=True):
        ...
        self.obstacles = [[3,4], [3,5], [4,4], [4,5], [4,6], [5,4], [5,5], [5,6], [6,4], [6,5], [6,6]]
        ...
        # set all cells to non visited
        self.states = np.zeros(maze_size, dtype=int)
        self.states[0][0] = 1
    ...
    def move_robot(self, dir):
        ...
        if self.__maze.is_open(self.__robot, dir):
            self.__colour_cell(self.__robot, colour=(0,255,0), transparency=255)
            # move the robot
    ...
    def __draw_maze(self):
        ...
        for x in range(len(self.maze.maze_cells)):
            ...
            for y in range (len(self.maze.maze_cells[x])):
                ...
                #print(walls_status, x,y,dirs)

    def __cover_walls(self, x, y, dirs, colour=(255, 255, 255, 255)):
        ...
        for dir in dirs:
            ...
            pygame.draw.line(self.maze_layer, colour, line_head, line_tail, 5)
    ...
    def __draw_robot(self, colour=(0, 0, 255), transparency=255):
        ...
        r = int(min(self.CELL_W, self.CELL_H)/2.5 + 0.5)

        pygame.draw.circle(self.maze_layer, colour + (transparency,), (x, y), r)

    def __draw_entrance(self, colour=(0, 255, 0), transparency=255):
        self.__colour_cell(self.entrance, colour=colour, transparency=transparency)

    def __draw_goal(self, colour=(255, 0, 0), transparency=255):
        #print(self.goal)
        self.__colour_cell(self.goal, colour=colour, transparency=transparency)

    def __draw_obstacles(self, colour=(0, 0, 0), transparency=255):
        for elem in self.obstacles:
            #time.sleep(2)
            if self.states[elem[0],elem[1]]==0:
                self.__colour_cell(elem, colour=colour, transparency=transparency)
            else:
                self.__colour_cell(elem, colour=(255,0,0), transparency=transparency)
    ...
    def __colour_cell(self, cell, colour, transparency):
        ...
        x = int(cell[0] * self.CELL_W + 0.5)
        y = int(cell[1] * self.CELL_H + 0.5)
...
class Maze:
    ...
    def _generate_maze(self):
        ...
        if self.has_loops:
            self.__break_random_walls(0.99)
